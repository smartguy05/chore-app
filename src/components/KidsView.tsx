...
const EditKidModal: React.FC<EditKidModalProps> = ({ kid, onClose, onKidUpdated }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (newPin.trim()) {
        kidData.pin = newPin.trim();
      }

      await axios.put(`/api/parent/kids/${kid.id}`, kidData);
              ...
              {showPinField && (
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-gray-500">
                    Enter a new 4-digit PIN for {kid.name} to use when logging in.
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="flex gap-3 pt-4">
              ...
              >
                {loading ? (
                  ...
                  newPin.trim() ? 'Update Kid & Reset PIN' : 'Update Kid'
                )}
              </button>
            </div>
          </form>
